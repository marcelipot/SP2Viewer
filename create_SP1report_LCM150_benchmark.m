...
if strcmpi(source, 'display') == 1;
    ...
    if window_size(3) < 1140 | window_size(4) < 605;
        h = warndlg('Screen resolution below [1140 605]. Display could be impacted', 'Warning');
        waitfor(h);
    end;
...
if strcmpi(answerReport, 'SP1') == 1 | strcmpi(answerReport, 'GE') == 1;
    %its a 150m so that short intervals
...
%Breath 150m, long intervals take col=3
...
DPSTOT = [];
if strcmpi(answerReport, 'SP1') == 1 | strcmpi(answerReport, 'GE') == 1;
    %it's a 150 so take col = 7 for short intervals
    colinterest_DPS = 7;
...
elseif strcmpi(answerReport, 'SP2') == 1;
    %it's a 150 so take col = 10 for short intervals
    validIndex = [5 10 15 20 25 30];
    DPSLap = [];
    interpDPS = 0;
    colinterest_DPS = 10;
...
if strcmpi(answerReport, 'SP1') == 1 | strcmpi(answerReport, 'GE') == 1;
    ...
    %It's a 150m so take col = 3 for short intervals
    validIndex = [5 10 15 20 25 30];
    SRLap = [];
    interpSR = 0;
    colinterest_SR = 3;
...
elseif strcmpi(answerReport, 'SP2') == 1;
    ...
    %It's a 150m so take col = 7 for short intervals
    validIndex = [5 10 15 20 25 30];
    SRLap = [];
    interpSR = 0;
    colinterest_SR = 7;
...
txtBlack_pdf = uicontrol('parent', hdef, 'Style', 'Text', 'Visible', 'on', 'units', 'pixels', ...
    'position', [30, window_size(4)-610, 1140, 1], 'HorizontalAlignment', 'left', ...
