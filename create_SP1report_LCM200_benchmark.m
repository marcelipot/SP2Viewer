...
if strcmpi(source, 'display') == 1;
    ...
    if window_size(3) < 1140 | window_size(4) < 670;
        h = warndlg('Screen resolution below [1140 670]. Display could be impacted', 'Warning');
        waitfor(h);
    end;
...
position1INI = [568, window_size(4)-430, 62, 30];
position2INI = [568, window_size(4)-425, 62, 20];
ColorOdd = [0.97 0.96 0.98];
ColorEven = [0.85 0.89 0.93];
ColorEnd = [0.95 0.95 0.83];
iter = 0;
...
for i = 1:length(KeyDist)+1;
    ...
    if i == length(KeyDist)+1;
        txtCol9a_pdf = uicontrol('parent', hdef, 'Style', 'Text', 'Visible', 'on', 'units', 'pixels', ...
            'position', [568, window_size(4)-670, 62, 30], 'HorizontalAlignment', 'left', ...
            'BackgroundColor', ColorEnd, 'ForegroundColor', [0 0 0], 'FontName', 'Arial', ...
            'FontWeight', 'Normal', 'Fontsize', font4, 'String', '');
        set(txtCol9a_pdf, 'fontunits', 'normalized', 'units', 'normalized');
        txtCol9A_pdf = uicontrol('parent', hdef, 'Style', 'Text', 'Visible', 'on', 'units', 'pixels', ...
            'position', [568, window_size(4)-665, 62, 20], 'HorizontalAlignment', 'center', ...
            'BackgroundColor', ColorEnd, 'ForegroundColor', [0 0 0], 'FontName', 'Arial', ...
