...
if (handles.animationsplitsTimeEC + 1) > nbFrameMax;
    return;
end;
...
liTime = find(isnan(TimeMain(end,:)) == 0);
CurrentTime = TimeMain(handles.animationsplitsTimeEC,liTime(1));
...
while proceed == 1;
    ...
    %jump: 2 frames by 2 frames
    handles.animationsplitsTimeEC = handles.animationsplitsTimeEC + jump;
    if handles.animationsplitsTimeEC > nbFrameMax;
        handles.animationsplitsTimeEC = nbFrameMax;
    end;
    CurrentTime = CurrentTime + (jump.*(1./FrameRate));
    ...
    for race = 1:nbRaces;
        ...
        if CurrentTime <= RaceTimeCorr;
            ...
            if isnan(vectorDistanceAW(handles.animationsplitsTimeEC)) == 0;
                ...
                %under water line only to be updated
                try;
    ...
    if (handles.animationsplitsTimeEC + 1) > nbFrameMax;
        proceed = 0;
        set(handles.AminationStatus_analyser, 'String', 'Stop');
    end;
    ...
    t(iterMain) = toc;
    if iterMain > 5;
        AvTime = mean(t(iterMain-5:iterMain));
        jump = ceil(AvTime.*FrameRate)+2;
    end;
    if t(iterMain) < (jump./FrameRate);
        pause((jump./FrameRate)-t(iterMain));
    end;
    iterMain = iterMain + 1;
end;

StatusEC = get(handles.AminationStatus_analyser, 'String');
if strcmpi(StatusEC, 'Stop');
    splits3D_Stop_analyser;
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    
elseif strcmpi(StatusEC, 'PrevFrame');
    splits3D_PrevFrame_analyser;
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    
elseif strcmpi(StatusEC, 'PrevSplit');
    splits3D_PrevSplit_analyser;
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    
elseif strcmpi(StatusEC, 'NextFrame');
    splits3D_NextFrame_analyser;
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    
elseif strcmpi(StatusEC, 'NextSplit');
    splits3D_NextSplit_analyser;
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    
elseif strcmpi(StatusEC, 'ShowSplit');
    showsplits_analyser;
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    
elseif strcmpi(StatusEC, 'PopRaceLap');
    popupRaceLap_analyser;
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);

elseif strcmpi(StatusEC, 'PopSplitsDist');
    popupsplits_analyser;
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    
elseif strcmpi(StatusEC, 'SplitsOff');
    checksplitsoff_analyser;
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    
elseif strcmpi(StatusEC, 'SplitsPercentage');
    checksplitspercentage_analyser;
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);

elseif strcmpi(StatusEC, 'SplitsTime');
    checksplitstime_analyser;
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    
elseif strcmpi(StatusEC, 'SplitsCheck1D');
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    set(handles.splits1D_check_analyser, 'Value', 0);
    set(handles.splits3D_check_analyser, 'Value', 1);
    andles.CurrentvalueSplits1D = 0;
    andles.CurrentvalueSplits3D = 1;
    herror = errordlg('Impossible to update while animation playing', 'Error');
    asOnTop = WinOnTop(herror);
    
elseif strcmpi(StatusEC, 'SplitsCheck2D');
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    set(handles.splits2D_check_analyser, 'Value', 0);
    set(handles.splits3D_check_analyser, 'Value', 1);
    andles.CurrentvalueSplits2D = 0;
    andles.CurrentvalueSplits3D = 1;
    herror = errordlg('Impossible to update while animation playing', 'Error');
    asOnTop = WinOnTop(herror);
    
elseif strcmpi(StatusEC, 'SplitsCheck3D');
    checksplits3D_analyser
    set(allchild(handles.Play_button_analyser), 'Visible', 'off');
    
elseif strcmpi(StatusEC, 'DataPanel');
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    herror = errordlg('Impossible to update while animation playing', 'Error');
    asOnTop = WinOnTop(herror);

elseif strcmpi(StatusEC, 'SkillPanel');
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    herror = errordlg('Impossible to update while animation playing', 'Error');
    asOnTop = WinOnTop(herror);

elseif strcmpi(StatusEC, 'PacingPanel');
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    herror = errordlg('Impossible to update while animation playing', 'Error');
    asOnTop = WinOnTop(herror);
    
elseif strcmpi(StatusEC, 'UpdateAnimation');
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    herror = errordlg('Impossible to update while animation playing', 'Error');
    asOnTop = WinOnTop(herror);
    
elseif strcmpi(StatusEC, 'DisplayData');
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    herror = errordlg('Impossible to update while animation playing', 'Error');
    asOnTop = WinOnTop(herror);
    
elseif strcmpi(StatusEC, 'Other');
    axes(handles.Play_button_analyser); imshow(handles.icones.play_offb);
    herror = errordlg('Impossible to update while animation playing', 'Error');
    asOnTop = WinOnTop(herror);
