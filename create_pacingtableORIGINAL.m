...
for i = 3:nbRaces+2;    
    ...
    if strcmpi(detailRelay, 'None') == 1;
        graphTitle = {Athletename; [Meet Year]; [num2str(RaceDist) 'm-' StrokeType]; Stage; SourceName};
        graphTitleBis = {Athletename; [Meet Year]; [num2str(RaceDist) 'm-' StrokeType]; Stage; [timeSecToStr(SplitsAll(end,2)) ' (' SourceName ')']};
    else;
        graphTitle = {Athletename; [Meet Year]; [num2str(RaceDist) 'm-' StrokeType]; [Stage '-' detailRelay ' ' valRelay]; SourceName};
        graphTitleBis = {Athletename; [Meet Year]; [num2str(RaceDist) 'm-' StrokeType]; [Stage '-' detailRelay ' ' valRelay]; [timeSecToStr(SplitsAll(end,2)) ' (' SourceName ')']};
    end;
    ...
    YearShort = Year(3:4);
    if strcmpi(detailRelay, 'None') == 1;
        graphTitle2 = [Athletename ' ' MeetShort YearShort ' ' StageShort ' (' SourceName ')'];
    else;
        graphTitle2 = [Athletename ' ' MeetShort YearShort ' ' StageShort '-' detailRelay ' (' SourceName ')'];
    end;
