function [] = splits3D_PrevSplit_analyser(varargin);
...
if strcmpi(StatusEC, 'Play') == 1;
    ...
    if handles.animationsplitsTimeEC == 1;
        return;
    end;
    ...
    posY = handles.posY;
    % handles.animationsplitsTimeEC = handles.animationsplitsTimeEC - 1;
    ...
    if isempty(DistSplitEC) == 1;
        ...
        DistSplitECend = DistSplitEC(end);
        ...
        if rowPrev == handles.animationsplitsTimeEC;
            if length(DistSplitEC) == 1;
                ...
                DistSplitECend = DistSplitEC(end-1);
                [~, LocMinEC] = min(abs(DistanceMain_Interp(:,raceLeader)-DistSplitECend));
                if DistanceMain_Interp(LocMinEC,raceLeader) < DistSplitECend;
                    handles.animationsplitsTimeEC = LocMinEC + 1;
                else;
...
set(handles.AminationStatus_analyser, 'String', 'PrevSplit');
