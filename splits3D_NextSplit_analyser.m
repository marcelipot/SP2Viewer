function [] = splits3D_NextSplit_analyser(varargin);
...
if strcmpi(StatusEC, 'Play') == 1;
    ...
    %Video already stopped... bring it back to  the beginning
    ...
    [leaderPos, laneleader] = max(laneOrder(:,3));
    raceLeader = laneOrder(laneleader(1),2);
    if isfield(handles, 'currentLeaderAnimation') == 1
        raceLeaderPrev = handles.currentLeaderAnimation;
    else;
        raceLeaderPrev = raceLeader;
    end;
    handles.currentLeaderAnimation = raceLeader;

    DistSplitEC = dataMatSplitsPacingFullRace(dataMatSplitsPacingFullRace(:,1) >= leaderPos, 1);

    rowPrev = handles.animationsplitsTimeEC;
    if isempty(DistSplitEC) == 1;
        handles.animationsplitsTimeEC = nbFrameMax;
    else;
        DistSplitECend = DistSplitEC(1);    
        distrealPrev = DistanceMain_Interp(handles.animationsplitsTimeEC,raceLeader);
        [~, LocMinEC] = min(abs(DistanceMain_Interp(:,raceLeader)-DistSplitECend));

        if DistanceMain_Interp(LocMinEC,raceLeader) < DistSplitECend;
            handles.animationsplitsTimeEC = LocMinEC + 1;
        else;
            handles.animationsplitsTimeEC = LocMinEC;
        end;

        if rowPrev == handles.animationsplitsTimeEC;
            if length(DistSplitEC) == 1;
                handles.animationsplitsTimeEC = nbFrameMax;
            else;
                DistSplitECend = DistSplitEC(2);
                [~, LocMinEC] = min(abs(DistanceMain_Interp(:,raceLeader)-DistSplitECend));
                if DistanceMain_Interp(LocMinEC,raceLeader) < DistSplitECend;
                    handles.animationsplitsTimeEC = LocMinEC + 1;
                else;
            ...
            if raceLeaderPrev ~= raceLeader;
                distreal = DistanceMain_Interp(handles.animationsplitsTimeEC,raceLeader);
                if distrealPrev+3 > distreal;
                    if length(DistSplitEC) == 1;
                        handles.animationsplitsTimeEC = nbFrameMax;
                    else;
                        DistSplitECend = DistSplitEC(2);
    ...
    for race = 1:nbRaces;
        ...
        lilim = floor((RaceTime-5)*FrameRate);
        if mod(lilim, 1) == 0;
            lilim = lilim-1;
        end;
...
set(handles.AminationStatus_analyser, 'String', 'NextSplit');
